...
function Gallery({ categories, works, onWorkClick }) {
  ...
  const buttonStyle =
    'px-4 py-2 rounded-full text-sm font-semibold transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800 shadow-sm';
        ...
        {filteredWorks.map((work, index) => (
          ...
          >
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent transition-all duration-300 flex items-end p-4">
              <h3 className="text-white text-lg font-bold opacity-0 group-hover:opacity-100 transform group-hover:translate-y-0 translate-y-4 transition-all duration-300">
                {work.title}
              </h3>
            </div>
          </motion.div>
